...
iree_status_t iree_hal_amdgpu_access_agent_list_resolve_scope_agents(
  ...
  if (access_scope == IREE_HAL_VIRTUAL_MEMORY_ACCESS_SCOPE_DEVICE) {
    return iree_hal_amdgpu_access_agent_list_resolve_queue_agents(
        topology, queue_affinity_domain, queue_affinity, out_agent_list);
  }
  if (access_scope == IREE_HAL_VIRTUAL_MEMORY_ACCESS_SCOPE_ALL) {
